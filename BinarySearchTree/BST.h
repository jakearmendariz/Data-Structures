...
//Calls the other methids to delete, returns a 0 if complete and -1 if not
int deleteItem(char *term_to_delete, BSTObj **pT);

//Actually frees the node
void deleteNode(BSTObj *T);

///term is the term whos parent you find
//@returns the parent node to the term that needs to be deleted
BSTObj *retrieveNode(char *term, BSTObj *T);

//Returns the leftmost element
BSTObj *findLeftMost(BSTObj *root);

//Helper method, simply finds which side, left or right the node is on
